	...
	**********************************************************************************************/
	...
	protected void AddTheEntryToTheBottom() {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://localhost:3306/pets?", "root","Naven@30");
			java.sql.Statement stat;
		
			stat = con.createStatement(); // Creating statement 
			
			ObservableList<ListItemEntry> art = super.getTheList();
			
			for(String artifact: artifactsSelected) {
				String statement = "Update artifacts set ec_name ='" + art.get(tabSpecificIndex).getListItemName()+
						"' where name = '" + artifact+ "'"   ;
				stat.executeUpdate(statement); // Executing Query
		    
			}
			
			for(String defect: defectsSelected) {
				String statement = "Update defect set ec_name ='" + art.get(tabSpecificIndex).getListItemName()+
						"' where name = '" + defect+ "'"   ;
				stat.executeUpdate(statement); // Executing Query
		    
			}
			
			for(String plan: plansSelected) {
				String statement = "Update plan set ec_name ='" + art.get(tabSpecificIndex).getListItemName()+
						"' where name = '" + plan+ "'"   ;
				stat.executeUpdate(statement); // Executing Query
		    
			}
			
			for(String interuption: interruptionsSelected) {
				String statement = "Update interuption set ec_name ='" + art.get(tabSpecificIndex).getListItemName()+
						"' where name = '" + interuption+ "'"   ;
				stat.executeUpdate(statement); // Executing Query
		    
			}
		    
		    System.out.println("Successfull artifacts updated");
